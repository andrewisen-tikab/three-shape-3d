...
import type Shape3D from "../../src/core/Shape3D";
import type { AddObjectsOnShapeParams, Vertex } from "../../src/types";
...
export default class ObjectsOnShapeFactory extends THREE.EventDispatcher {
	...
	private getTotalObjects(
		...
		for (let i = 1; i < vertices.length; i++) {
			...
			const numberOfObjects = this.getNumberOfObjects(
				previousVertex,
				currentVertex,
				params,
			);
	...
	private addObjectOnLine(index: number, numberOfObjects: number): void {
		_position
			.copy(_previousVertex)
			.addScaledVector(_line, index / numberOfObjects);
