...
import type Shape3DFactory from "../../factory/Shape3DFactory";
import { getMidpoint, setLineAngle, setLineLength } from "../../utils";
import LabelsManager from "./LabelsManager";
import type {
	LastSelectedVertex,
	Mode,
	TransformShapeControlsGizmoParams,
} from "./types";
...
class TransformShapeControls extends THREE.Object3D<TransformShapeControlsEvents> {
	...
	private _vertexPlane: THREE.Mesh<
		THREE.PlaneGeometry,
	...
	pointerMove(pointer: PointerEvent) {
		...
		if (this.lastSelectedVertex) {
			...
			this.lastSelectedVertex.position
			...
			if (this.translationSnap) {
				this.lastSelectedVertex.position.x =
					Math.round(
						this.lastSelectedVertex.position.x / this.translationSnap,
					) * this.translationSnap;
				this.lastSelectedVertex.position.y =
					Math.round(
						this.lastSelectedVertex.position.y / this.translationSnap,
					) * this.translationSnap;

				this.lastSelectedVertex.position.z =
					Math.round(
						this.lastSelectedVertex.position.z / this.translationSnap,
					) * this.translationSnap;
			}
	...
	private addHandles() {
		...
		const currentIndex =
			(this.lastSelectedVertex?.userData as Partial<VertexMetadata>)?.index ??
			-99;
		...
		for (let index = 0; index < vertices.length; index++) {
			...
			const midpoint = new TransformShapeControls.VertexObject(
				this.domElement,
				{
					type: "midpoint",
				},
	...
	private updateLabels() {
		if (
			this.params.showLengthLabels === false &&
			this.params.showAngleLabels === false
		) {
	...
	private addGhostVertex(): void {
		this.ghostVertex = new TransformShapeControls.VertexObject(
			this.domElement,
			{
				type: "ghost",
			},
	...
	setLineLength(index: number, lineLength: number) {
		...
		const adjustedIndex =
			index === this.object!.getVertices().length ? 0 : index;
		const vertices = this.object!.getVertices();
	...
	setLineAngle(index: number, angleInDegrees: number) {
		...
		const adjustedIndex =
			index === this.object!.getVertices().length ? 0 : index + 1;
		const vertices = this.object!.getVertices();
...
export {
	...
	TransformShapeControlsGizmo,
	TransformShapeControlsPlane,
};
