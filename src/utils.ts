...
export const setOffsetPositionFromLine = (
	...
	_midpoint3A.add(
		_perpendicular.multiplyScalar(
			shouldFlip ? -offsetDistance : offsetDistance,
		),
	);
...
export const getLineRotationAsDeg = (
	firstVertex: Vertex,
	secondVertex: Vertex,
) => {
	_firstVertex.fromArray(firstVertex);
...
export const generateAngle = (
	...
	_line2A
		...
		.normalize();
	_line2B
		.set(nextVertex[0] - currentVertex[0], nextVertex[2] - currentVertex[2])
		.normalize();
	...
	const archRotationA1 = lineBHasPositiveSlope
		? Math.PI - angleB
		: -(Math.PI - angleB); // Should not flip.
	...
	const ellipseCurveRotationB1 = lineBHasPositiveSlope
		? -(Math.PI + angleB)
		: Math.PI + angleB; // Should flip.

	const ellipseCurveRotation = shouldFlip
		? ellipseCurveRotationB1
		: ellipseCurveRotationA1;
	...
	const humanReadableAngles = THREE.MathUtils.radToDeg(Math.abs(angle)).toFixed(
		2,
	);
	...
	inputElement.setAttribute(
		"size",
		`${inputElement.getAttribute("placeholder")!.length - 1}`,
	);
